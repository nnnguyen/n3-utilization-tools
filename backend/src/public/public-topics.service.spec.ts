...
describe("PublicTopicsService", () => {
  ...
  let tx: {
    ...
    wordAggregate: { upsert: jest.Mock };
  };
  ...
  describe("getPublicInfo", () => {
    it("throws NotFoundException when the topic does not exist", async () => {
      ...
      await expect(service.getPublicInfo("MISSING")).rejects.toBeInstanceOf(
        NotFoundException,
      );
    });
        ...
        currentQuestion: {
          ...
          config: {
            ...
            allowDuplicateFromSameUser:
              activeQuestion.allowDuplicateFromSameUser,
  ...
  describe("createResponse", () => {
    const dto = {
      text: "Sáng tạo",
      participantSessionId: "11111111-1111-1111-1111-111111111111",
    };

    it("throws NotFoundException when the question does not exist", async () => {
      prisma.question.findUnique.mockResolvedValue(null);
      await expect(
        service.createResponse("question-1", dto),
    ...
    it("does not broadcast when the submission is rejected", async () => {
      ...
      prisma.response.count.mockResolvedValue(0);
      await service
        .createResponse("question-1", { ...dto, text: "!!!" })
        .catch(() => undefined);
