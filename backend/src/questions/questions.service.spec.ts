...
describe("QuestionsService", () => {
  ...
  let wordCloudGateway: {
    broadcastSnapshot: jest.Mock;
    broadcastResultsRevealed: jest.Mock;
  };
  ...
  describe("create", () => {
    it("rejects when the topic belongs to another user", async () => {
      ...
      await expect(service.create(topicId, ownerId, {})).rejects.toBeInstanceOf(
        ForbiddenException,
      );
  ...
  describe("ownership via parent topic", () => {
    it("rejects update when the question belongs to another user", async () => {
      ...
      await expect(
        service.update(question.id, "someone-else", {}),
  ...
  describe("reorder", () => {
    it("rejects when orderedIds does not match the question set in the topic", async () => {
      ...
      await expect(
        service.reorder(topicId, ownerId, ["q1", "q3"]),
  ...
  describe("applySettingsToAll", () => {
    ...
    it("supports the showResponses group independently", async () => {
      ...
      await service.applySettingsToAll(question.id, ownerId, [
        ApplySettingsGroup.SHOW_RESPONSES,
      ]);
  ...
  describe("revealResults", () => {
    it("rejects when resultVisibility is not ON_CLICK", async () => {
      ...
      await expect(
        service.revealResults(question.id, ownerId),
    ...
    it("sets resultsRevealed = true and broadcasts the snapshot", async () => {
      ...
      expect(wordCloudGateway.broadcastResultsRevealed).toHaveBeenCalledWith(
        topicId,
        question.id,
      );
    });
  ...
  describe("getWordCloud", () => {
    it("delegates to WordCloudService once ownership is verified", async () => {
      ...
      await expect(service.getWordCloud(question.id, ownerId)).resolves.toEqual(
        snapshot,
      );
