...
describe("TopicsService", () => {
  ...
  describe("findOneForUser", () => {
    it("throws NotFoundException when the topic does not exist", async () => {
      ...
      await expect(
        service.findOneForUser("missing", ownerId),
    ...
    it("throws ForbiddenException when the topic belongs to another user", async () => {
      ...
      await expect(
        service.findOneForUser(topic.id, "someone-else"),
      ).rejects.toBeInstanceOf(ForbiddenException);
    });

    it("returns the topic when the current user is the owner", async () => {
      prisma.topic.findUnique.mockResolvedValue(topic);
      await expect(service.findOneForUser(topic.id, ownerId)).resolves.toEqual(
        topic,
      );
    });
  });
  ...
  describe("update / remove", () => {
    ...
    it("rejects remove when the topic belongs to another user", async () => {
      prisma.topic.findUnique.mockResolvedValue(topic);
      await expect(
        service.remove(topic.id, "someone-else"),
      ).rejects.toBeInstanceOf(ForbiddenException);
  ...
  describe("setCurrentQuestion", () => {
    ...
    it("rejects when the question does not belong to this topic", async () => {
      ...
      await expect(
        service.setCurrentQuestion(topic.id, ownerId, question.id),
      ).rejects.toThrow();
      expect(prisma.$transaction).not.toHaveBeenCalled();
    });
    ...
    it("sets currentQuestionId and opens the new question for responses", async () => {
      ...
      const tx = {
        question: {
          ...
          update: jest
            .fn()
            .mockResolvedValue({ ...question, status: "ACTIVE" }),
        },
        topic: {
          update: jest
            .fn()
            .mockResolvedValue({ ...topic, currentQuestionId: question.id }),
        },
      };
      ...
      const result = await service.setCurrentQuestion(
        topic.id,
        ownerId,
        question.id,
      );
      ...
      expect(wordCloudGateway.broadcastQuestionChanged).toHaveBeenCalledWith(
        topic.id,
        {
          ...question,
          status: "ACTIVE",
