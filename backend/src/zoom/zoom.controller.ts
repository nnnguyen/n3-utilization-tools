...
export class ZoomController {
  ...
  async handleWebhook(
    ...
    if (payload.event === 'endpoint.url_validation' && process.env.ZOOM_WEBHOOK_SECRET_TOKEN) {
    ...
    if (payload.event === 'recording.completed') {
      ...
      this.zoomService.handleRecordingCompleted(payload.payload)
        .catch(err => this.logger.error('Error processing recording in background', err.stack));
