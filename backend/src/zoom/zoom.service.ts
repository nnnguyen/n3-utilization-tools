...
export class ZoomService {
  ...
  private async getAccessToken(): Promise<string> {
    ...
    try {
      ...
      this.logger.error('Error fetching Zoom access token', error.response?.data || error.message);
      throw error;
    }
  }
  ...
  async listRecordings(userId: string = 'me') {
    ...
    try {
      const response = await firstValueFrom(
        this.httpService.get(`https://api.zoom.us/v2/users/${userId}/recordings`, {
          headers: {
      ...
      this.logger.error('Error listing Zoom recordings', error.response?.data || error.message);
      throw error;
    }
  }
