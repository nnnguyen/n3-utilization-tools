...
| P2-3 | Google Drive backup | new scope `drive.file` (re-authorization flow), upload MP4/M4A/VTT after sync, folder layout, retries, storage display | folder structure; which files — **decided 2026-09-27: the user chooses whether to save to Drive** (off by default, per recording too; files stay private). Design draft: [P2-3-google-drive.md](design/P2-3-google-drive.md), tasks P2-3a → P2-3c |
| P2-4 | Upcoming meetings | upcoming Zoom meetings with the rule/result that will apply (Zoom API, no Google scope) | **design draft revised 2026-09-27, awaiting approval**: [P2-4-upcoming-meetings.md](design/P2-4-upcoming-meetings.md), task P2-4a. Auto-attaching videos to Calendar events **dropped**: only the super admin decides whether and with whom videos/recordings are shared |
...
| P2-6 | Zalo OA notifications | on top of P2-1 | verified OA available? — **design draft revised 2026-09-27**: [P2-6-zalo-oa.md](design/P2-6-zalo-oa.md); the super admin approves every announcement (no automatic sending); audience and a verified OA still to be decided |
...
### P2-5 subtasks (approved 2026-09-26)

Design: [docs/design/P2-5-captions.md](design/P2-5-captions.md) — approved with the proposed answers: default language `vi` (rules can override), off by default, captions published (not drafts), built before P2-2, older videos only through an "Upload captions" button. Specs in §8 of the design note.
...
Design: [docs/design/P2-8-posthog.md](design/P2-8-posthog.md) — **approved 2026-09-26** with the proposed answers (activation funnel + sync failures first; no session replay; consent asked once after sign-in, no tracking until agreed; EU region; Admin link, no embedding yet). Subtasks P2-8a ✅ (consent + frontend SDK + `/ingest` proxy), P2-8b ✅ (backend events), P2-8c ✅ (frontend events + admin link) are specified in §7 of the design note; the human creates the PostHog project and sets the keys. **Must cover**: the questions to answer (e.g. sign-up → connect YouTube → first sync funnel, feature usage, Zoom rules, Word Cloud sessions); event list, with reliable events sent from the backend (`posthog-node`: sync completed/failed, webhook received, auto-upload skipped) and UI events from `posthog-js`; identity by internal user id (never email); privacy — no capture on the Integrations drawer and the Admin page (secrets, temp passwords), no persons/replay on public Word Cloud audience pages, consent notice (Vietnam Decree 13/2023, GDPR if EU users); region (US/EU, fixed at sign-up); reverse proxy through a Next.js rewrite; feature flags (could replace env switches like `CONNECTIONS_READ`); dashboards embedded in the Admin page for super admins. **Open questions (ask)**: what to learn first; session replay yes/no; consent approach; region.
