...
**Trạng thái**: ✅ Approved (25/09/2026 — dùng các phương án đề xuất ở mục 7, gồm super admin và dịch vụ email ở mục 6b–6c) · Liên quan: [ROADMAP §2.3](../ROADMAP.md), [P2-1](P2-1-connector.md), [IMPLEMENTATION_PLAN.ai.md §4](../IMPLEMENTATION_PLAN.ai.md)
...
- **Trang Quản trị → Tài khoản**: danh sách, tìm kiếm; khoá/mở; xác thực/bỏ xác thực email; cấp/thu super admin; gửi lại mật khẩu tạm; xoá (xác nhận). **Quản trị → Workspace**: danh sách, tạo, đổi tên, xoá, quản lý thành viên của bất kỳ workspace nào.
- **Thêm người** (3 cách): mời qua **email**, **link mời**, hoặc **tạo thủ công**: nhập email, tên, (tuỳ chọn) workspace + vai trò → hệ thống **tự sinh mật khẩu tạm** (super admin không xem được) → tuỳ chọn "đánh dấu email đã xác thực" (mặc định bật) và "bắt buộc đổi mật khẩu lần đầu" (mặc định bật) → **"Tạo và gửi email"**. Khi còn `mustChangePassword`, đăng nhập xong chỉ vào được màn hình đổi mật khẩu (API khác trả 403). Không bắt buộc đổi thì mật khẩu tạm **hết hạn sau 30 ngày** nếu chưa tự đổi. "Gửi lại mật khẩu tạm mới" làm mật khẩu cũ mất hiệu lực. **Dự phòng khi chưa cấu hình email**: nút gửi bị khoá, super admin chọn "hiện mật khẩu một lần để copy".
...
## 6c. Dịch vụ gửi email

Railway chỉ cho SMTP ra ngoài ở gói Pro trở lên (Free/Trial/Hobby bị chặn), nên production hiện **không gửi được email nào** (xác thực, quên mật khẩu, thông báo). Chuyển `MailService` sang **Resend** (HTTP API): env `RESEND_API_KEY`, `MAIL_FROM` (địa chỉ trên domain đã xác minh DNS, ví dụ `n3-utils.com`); giữ SMTP làm dự phòng khi chạy local/gói Pro. Người dùng cần: tạo tài khoản Resend, thêm bản ghi DNS, đặt biến trên Railway. Có thể làm **trước** các task workspace (không phụ thuộc P2-1f).
...
| Mật khẩu tạm nằm trong hộp thư | Mặc định buộc đổi lần đầu; không buộc thì hết hạn 30 ngày; gửi lại vô hiệu mật khẩu cũ |
...
3. **Mời bằng gì** — Đề xuất: **email** (qua Resend, mục 6c) + **link mời** để copy gửi qua Zalo/Messenger; người đã có tài khoản nhận thêm thông báo trong chuông. Super admin còn có "tạo thủ công" (mục 6b).
...
9. **Dịch vụ email** — Đề xuất: **Resend** (mục 6c).

## 8. Các task triển khai

Mỗi task theo Definition of done của IMPLEMENTATION_PLAN.ai.md. P2-2a → P2-2h làm sau P2-1f; **P2-2i làm được ngay**. Thứ tự đề xuất: i → a → b → c → h → d → e → f → g.
...
**P2-2h — Super admin and system admin pages.** *Changes*: `User.platformRole`, `isLocked`, `mustChangePassword`, `tempPasswordExpiresAt`; bootstrap from `BOOTSTRAP_SUPER_ADMIN_EMAIL` only while no super admin exists; `SuperAdminGuard`; admin API + pages (accounts: list/search, lock, verify, grant/revoke super admin, resend temp password, delete; workspaces: list/create/rename/delete/members of any workspace); manual account creation with generated temp password emailed (fallback: show once), must-change-password gate (all other APIs 403), 30-day expiry when not forced; locked accounts cannot log in; every action in `ActivityLog`. *Tests*: bootstrap only when zero super admins; last super admin cannot be removed; must-change gate; expiry; locked login refused; non-admins get 403. *Acceptance*: a super admin creates an account that receives its temp password by email and must change it at first login.

**P2-2i — Email through Resend (can run first).** *Changes*: `MailService` sends through the Resend HTTP API when `RESEND_API_KEY` is set (`MAIL_FROM` sender), SMTP kept as fallback; env in `.env.example`/README/docker-compose; "email configured" reflects either transport. *Tests*: transport selection; request payload (mocked fetch). *Acceptance*: on production, verification, password-reset and notification emails arrive.
