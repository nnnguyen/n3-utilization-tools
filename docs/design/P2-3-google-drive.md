...
**Trạng thái**: 📝 Bản nháp — chờ duyệt (26/09/2026) · Liên quan: [ROADMAP §3](../ROADMAP.md), [IMPLEMENTATION_PLAN.ai.md §4](../IMPLEMENTATION_PLAN.ai.md), [P2-1 Connector](P2-1-connector.md), [P2-5 Phụ đề](P2-5-captions.md)
...
Ngoài phạm vi: xoá recording trên Zoom, podcast/RSS, đồng bộ hai chiều, sao lưu video upload thủ công.
...
1. **Kích hoạt**: khi `recording.completed` đến (P1-3/P1-4) và `driveBackupEnabled` → tạo các dòng `DriveBackup` `pending`. **Không phụ thuộc YouTube**: vẫn sao lưu khi tắt tự upload hoặc khi sync YouTube lỗi (chính là lúc bản sao lưu có ích). Recording cũ: nút "Sao lưu vào Drive" từng dòng.
...
- Form Automation Workflow: công tắc **"Tự động sao lưu vào Google Drive"**, chọn loại file, nút mở thư mục trên Drive, dung lượng Drive còn trống (`about.get` → `storageQuota`).
...
3. **Bật mặc định** — **Tắt**, người dùng tự bật (tốn dung lượng Drive và băng thông).
...
**P2-3c — Giao diện.** Công tắc + loại file + dung lượng trong Automation Workflow, nhãn Drive và nút trong bảng recording, lịch sử sync, i18n vi/en, sự kiện PostHog. *Acceptance*: bật, recording mới tự lên Drive; nút sao lưu được một recording cũ.
